...
mod tests {
    use super::*;
    use common::chain::{NetUpgradeError, UpgradeVersion};

    impl TryFrom<(UpgradeVersion, ChainType)> for Config {
        type Error = NetUpgradeError;

        fn try_from(value: (UpgradeVersion, ChainType)) -> Result<Self, Self::Error> {
            if value.0 == UpgradeVersion::PoW {
                return Ok(Config::from(value.1));
            }
    ...
    fn check_from_chain_type() {
        let cfg = Config::from(ChainType::Mainnet);
        assert!(!cfg.no_retargeting);
        assert!(!cfg.allow_min_difficulty_blocks);

        let cfg = Config::from(ChainType::Regtest);
        assert!(cfg.no_retargeting);
        assert!(cfg.allow_min_difficulty_blocks);

        let cfg = Config::from(ChainType::Testnet);
        assert!(!cfg.no_retargeting);
        assert!(cfg.allow_min_difficulty_blocks);
    }
    ...
    fn check_generate_config() {
        ...
        fn check(vers: UpgradeVersion, chain: ChainType) {
            if let Ok(res) = Config::try_from((vers, chain)) {
                assert_eq!(Config::from(chain), res);
            } else {
    ...
    fn check_failed_generate_config() {
        fn check(vers: UpgradeVersion, chain: ChainType) {
            assert!(Config::try_from((vers, chain)).is_err())
        }
