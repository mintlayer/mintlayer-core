...
fn test_pow() {
    ...
    // checks
    let mut random_invalid_block =
        btf.random_block(btf.blocks.last().expect("genesis should be there"), None);
...
fn test_get_headers_different_chains() {
    common::concurrency::model(|| {
        ...
        // verify that the first header attaches to genesis
        {
            ...
            btf.create_chain(&btf.genesis().get_id(), (limit / 10) as usize, produce_test_block);
        ...
        // Verify that the first returned header attaches to the other chain
        {
            ...
            btf.create_chain(&btf.blocks[common_height - 1].get_id(), limit, produce_test_block);
            let locator = btf.consensus.get_locator().unwrap();
            btf.create_chain(&btf.blocks[common_height - 1].get_id(), limit * 4, produce_test_block);
        ...
        // Verify that the first returned header attaches before genesis
        {
            ...
            btf1.create_chain(&btf1.blocks[btf1.blocks.len() - 1].get_id(), limit, produce_test_block);
            btf2.create_chain(&btf2.blocks[btf2.blocks.len() - 1].get_id(), limit * 2, produce_test_block);
