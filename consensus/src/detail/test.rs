...
fn test_get_locator() {
    common::concurrency::model(|| {
        ...
        for _ in 0..limit {
            let new_block = produce_test_block(&consensus.chain_config, &prev_block, false);
            consensus.process_block(new_block.clone(), BlockSource::Peer(1)).ok().flatten().unwrap();
            prev_block = new_block;
        }
        ...
        let height = consensus.get_block_height_in_main_chain(&prev_block.get_id()).unwrap().unwrap();
