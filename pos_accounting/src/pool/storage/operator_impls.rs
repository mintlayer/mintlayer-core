    ...
    for PoSAccountingDB<S, T>
{
    fn create_pool(
        ...
        self.store.set_pool_owner_balance(pool_id, pool_data.pledge_amount())?;
    ...
    fn undo(&mut self, undo_data: PoSAccountingUndo) -> Result<(), Error> {
        match undo_data {
            ...
            PoSAccountingUndo::IncreasePoolOwnerBalance(undo) => {
                self.undo_increase_pool_balance(undo)
            }
        }
    }
}
...
impl<S: PoSAccountingStorageWrite<T>, T: StorageTag> PoSAccountingDB<S, T> {
    fn undo_create_pool(&mut self, undo: CreatePoolUndo) -> Result<(), Error> {
        ...
        self.store.del_pool_owner_balance(undo.pool_id)?;
    ...
    fn undo_decommission_pool(&mut self, undo: DecommissionPoolUndo) -> Result<(), Error> {
        ...
        self.store.set_pool_owner_balance(undo.pool_id, data_undo.pledge_amount())?;
