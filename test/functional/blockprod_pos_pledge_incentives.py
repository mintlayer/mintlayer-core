...
class StakingIncentivesTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        pool_1_balance_initial = int(self.nodes[0].chainstate_stake_pool_balance(
            pool_id_1_hex)['atoms'])
        self.log.debug("Pool 1 original balance {}".format(pool_1_balance_initial))

        pool_id_2_hex = ScaleBytes(bytes(pool_id_2)).to_hex()[2:]
        pool_2_balance_initial = int(self.nodes[0].chainstate_stake_pool_balance(
            pool_id_2_hex)['atoms'])
        ...
        pool_1_reward = int(self.nodes[0].chainstate_stake_pool_balance(
            pool_id_1_hex)['atoms']) - pool_1_balance_initial
        self.log.debug("Pool 1 reward {}".format(pool_1_reward))
        pool_2_reward = int(self.nodes[0].chainstate_stake_pool_balance(
