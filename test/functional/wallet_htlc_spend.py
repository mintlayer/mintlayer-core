...
class WalletHtlcSpend(BitcoinTestFramework):
    ...
    async def async_test(self):
        ...
        async with WalletRpcController(node, self.config, self.log) as wallet:
            ...
            alice_htlc_tx_id = await wallet.create_htlc(alice_amount_to_swap, token_id, alice_secret_hash, bob_address, refund_address, 2)
            self.generate_block()
            assert_in("Success", await wallet.sync())
            ...
            bob_htlc_tx_id = await wallet.create_htlc(bob_amount_to_swap, None, alice_secret_hash, alice_address, refund_address, 2)
            self.generate_block()
            assert_in("Success", await wallet.sync())
