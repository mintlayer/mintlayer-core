...
class WalletConflictTransaction(BitcoinTestFramework):
    ...
    def generate_block(self, transactions = []):
        ...
        strategy = "FillSpaceFromMempool" if not transactions else "LeaveEmptySpace"
        block = node.blockprod_generate_block(block_input_data, transactions, [], strategy)
    ...
    async def async_test(self):
        ...
        async with WalletCliController(node, self.config, self.log) as wallet:
            ...
            block_id = self.generate_block([freeze_tx])
            self.log.info(f"block with freeze tx {block_id}")
            assert_in("Success", await wallet.sync())
            ...
            assert not node.mempool_contains_orphan_tx(transfer_tx_id)

            assert_in("Success", await wallet.select_account(DEFAULT_ACCOUNT_INDEX))
            assert_in("The transaction was submitted successfully", await wallet.unfreeze_token(token_id))
            block_id = self.generate_block()
            self.log.info(f"block with un-freeze tx {block_id}")
            assert_in("Success", await wallet.sync())

            # check it is not in the mempool or in orphans
            assert not node.mempool_contains_tx(transfer_tx_id)
            assert not node.mempool_contains_orphan_tx(transfer_tx_id)
