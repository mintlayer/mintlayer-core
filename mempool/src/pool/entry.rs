...
use common::{
    chain::{SignedTransaction, Transaction, UtxoOutPoint},
...
impl TxEntry {
    ...
    pub fn utxo_outpoints(&self) -> impl Iterator<Item = &UtxoOutPoint> + '_ {
        self.transaction().inputs().iter().filter_map(|i| i.utxo_outpoint())
    }
}
