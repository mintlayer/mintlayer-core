...
use p2p::{
    ...
    net::{
        ...
        Event, GossipsubTopic, NetworkService,
    },
    P2P,
};
...
async fn test_libp2p_gossipsub() {
    ...
    let mut server1 =
        Libp2pService::new(addr1, &[], &[GossipsubTopic::Transactions]).await.unwrap();

    let addr2: Multiaddr = test_utils::make_address("/ip6/::1/tcp/");
    let mut server2 =
        Libp2pService::new(addr2, &[], &[GossipsubTopic::Transactions]).await.unwrap();
    ...
    // try to publish data on a topic with no peers
    assert_eq!(
        server1
            .publish(
