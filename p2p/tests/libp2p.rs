...
//         Libp2pService::start(addr1, &[], &[PubSubTopic::Transactions], Arc::clone(&config)).await.unwrap();
//     let addr2: Multiaddr = test_utils::make_address("/ip6/::1/tcp/");
//     let (mut conn2, mut pubsub2) =
//         Libp2pService::start(addr2, &[], &[PubSubTopic::Transactions], Arc::clone(&config)).await.unwrap();
...
//             let addr: Multiaddr = test_utils::make_address("/ip6/::1/tcp/");
//             let res = Libp2pService::start(addr, &[], &[PubSubTopic::Transactions], Arc::clone(&config)).await.unwrap();
//             (res.0, res.1)
//         }))
//         .await;
