...
use crate::{
    message::{BlockSyncMessage, HeaderList, TxnSyncMessage},
...
pub struct TestNode {
    ...
    txn_sync_msg_receiver: UnboundedReceiver<(PeerId, TxnSyncMessage)>,
...
impl TestNode {
    ...
    pub async fn start_with_params(
        ...
        let (txn_sync_msg_sender, txn_sync_msg_receiver) = mpsc::unbounded_channel();
    ...
    pub fn try_connect_peer(
        ...
        let (block_sync_msg_tx, block_sync_msg_rx) = mpsc::channel(20);
        let (txn_sync_msg_tx, txn_sync_msg_rx) = mpsc::channel(20);
        ...
        self.syncing_event_sender
            ...
            .unwrap();
        TestPeer::new(peer_id, block_sync_msg_tx, txn_sync_msg_tx)
    }
    ...
    pub async fn get_sent_txn_sync_message(&mut self) -> (PeerId, TxnSyncMessage) {
        expect_recv!(self.txn_sync_msg_receiver)
    }
    ...
    pub async fn assert_no_sync_message(&mut self) {
        let future = async {
            tokio::select! {
                ...
                _ = self.txn_sync_msg_receiver.recv() => {},
            }
        };
    ...
    pub async fn join_subsystem_manager(self) {
        ...
        drop(self.txn_sync_msg_receiver);
...
impl TestPeer {
    ...
    pub async fn send_block_sync_message(&self, message: BlockSyncMessage) {
        self.block_sync_msg_tx.send(message).await.unwrap();
    }

    pub async fn send_txn_sync_message(&self, message: TxnSyncMessage) {
        self.txn_sync_msg_tx.send(message).await.unwrap();
    }
...
struct MessagingHandleMock {
    ...
    txn_sync_msg_sender: UnboundedSender<(PeerId, TxnSyncMessage)>,
}
...
impl MessagingService for MessagingHandleMock {
    ...
    fn send_txn_sync_message(&mut self, peer: PeerId, message: TxnSyncMessage) -> Result<()> {
        self.txn_sync_msg_sender.send((peer, message)).unwrap();
        Ok(())
    }
}
