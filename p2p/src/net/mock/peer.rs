...
mod tests {
        ...
        T: MockTransport,
    {
        ...
        let mut server = T::bind(
            &<<T as MockTransport>::Stream as MockStream>::StreamKey::gen_new(),
            addr,
        )
        .await
        .unwrap();
        let client_key = <<T as MockTransport>::Stream as MockStream>::StreamKey::gen_new();
