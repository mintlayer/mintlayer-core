...
#[test]
fn num_active_peers() {
    let mut peerdb = PeerDb::<Libp2pService>::new(Arc::new(config::P2pConfigFile::new()));
...
fn adjust_peer_score_higher_threshold() {
    let mut config = config::P2pConfigFile::new();
...
fn peer_discovered_libp2p() {
    let mut peerdb = PeerDb::new(Arc::new(config::P2pConfigFile::new()));
