...
use chainstate::{ChainstateConfig, ban_score::BanScore};
use networking::{
    test_helpers::{
...
async fn unexpected_handshake_message<TTM>(seed: Seed)
where
...
async fn unexpected_handshake_message_tcp(#[case] seed: Seed) {
    run_with_timeout(unexpected_handshake_message::<TestTransportTcp>(seed)).await;
}
...
#[case(Seed::from_entropy())]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn unexpected_handshake_message_channels(#[case] seed: Seed) {
    run_with_timeout(unexpected_handshake_message::<TestTransportChannel>(seed)).await;
}
...
#[case(Seed::from_entropy())]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn unexpected_handshake_message_noise(#[case] seed: Seed) {
    run_with_timeout(unexpected_handshake_message::<TestTransportNoise>(seed)).await;
}
