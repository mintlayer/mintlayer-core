...
#[tokio::test]
async fn invalid_before_order_num() {
    let (task, response) = spawn_webserver("/api/v2/transaction?before_order_number=asd").await;
    ...
    assert_eq!(
        ...
        "Invalid transaction order number"
    );
...
#[tokio::test]
async fn invalid_offset_and_before_order_num() {
    let (task, response) =
        spawn_webserver("/api/v2/transaction?offset=1&before_order_number=1").await;
    ...
    assert_eq!(
        body["error"].as_str().unwrap(),
        "Cannot specify both offset and before order number"
    );
...
async fn ok(#[case] seed: Seed) {
    ...
    let task = tokio::spawn(async move {
        let web_server_state = {
            ...
            let chainstate_blocks = {
                ...
                // order_numbers start from 1 not 0
                num_txs += 1;
                ...
                let txs: Vec<serde_json::Value> = chainstate_block_ids
                    ...
                    .map(|(idx, ids)| {
                        ...
                        let order_num = num_txs - block.transactions().len() + transaction_index;
    ...
    // test before_order_number instead of offset
    let order_number = &expected_transactions[offset - 1]["order_number"].as_str().unwrap();
    eprintln!("order_number: '{order_number}'");
    let url = format!("/api/v2/transaction?before_order_number={order_number}&items={items}");
...
fn compare_body(body: &Value, expected_transaction: &Value) {
    ...
    assert_eq!(
        body.get("order_number").unwrap(),
        &expected_transaction["order_number"]
    );
}
