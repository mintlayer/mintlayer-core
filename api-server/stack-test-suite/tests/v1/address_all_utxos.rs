...
async fn invalid_address() {
    let (task, response) = spawn_webserver("/api/v1/address/invalid-address/all-utxos").await;
...
async fn address_not_found(#[case] seed: Seed) {
    ...
    let (task, response) =
        spawn_webserver(&format!("/api/v1/address/{}/all-utxos", address.get())).await;
...
async fn multiple_utxos_to_single_address(#[case] seed: Seed) {
    ...
    for (address, expected) in rx.await.unwrap() {
        let url = format!("/api/v1/address/{address}/all-utxos");
