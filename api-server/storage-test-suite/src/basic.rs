    ...
    F: Fn() -> Fut,
{
    ...
    // Test setting/getting transactions
    {
        ...
        let mut db_tx = storage.transaction_rw().await.unwrap();
        {
            ...
            let expected_last_order_number = 20;
            let start_order_number = 10;
            for i in start_order_number..=expected_last_order_number {
                let tx_id = H256::random_using(&mut rng);
                db_tx.set_transaction(tx_id.into(), i, owning_block1, &tx_info).await.unwrap();
            }

            let last_num = db_tx.get_last_transaction_order_number().await.unwrap();
            assert_eq!(last_num, expected_last_order_number);

            let take_txs = rng.gen_range(1..expected_last_order_number - start_order_number);
            let txs = db_tx
                .get_transactions_with_block_by_order_number(take_txs as u32, last_num + 1)
                .await
                .unwrap();
            assert_eq!(txs.len() as u64, take_txs);
            for (i, tx) in txs.iter().enumerate() {
                assert_eq!(tx.order_number, expected_last_order_number - i as u64);
            }
        }
