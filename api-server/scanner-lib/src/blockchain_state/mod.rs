...
use api_server_common::storage::storage_api::{
    ...
    ApiServerTransactionRw, Delegation,
};
use common::{
    address::Address,
    chain::{
...
async fn disconnect_tables_above_height<T: ApiServerStorageWrite>(
    ...
    while db_tx.get_best_block().await.expect("Unable to get best block").0 > block_height {
        let current_best = db_tx.get_best_block().await.expect("Unable to get best block");
    ...
    for output in outputs {
        match output {
            ...
            TxOutput::CreateStakePool(pool_id, stake_pool_data) => {
                if stake_pool_data.value() < chain_config.as_ref().min_stake_pool_pledge() {
                    // TODO turn all panic and expects back into errors
                    panic!("Not enough pledge to create stake pool")
                }
