...
impl Tab for WalletTab {
    ...
    fn content(&self, node_state: &NodeState) -> Element<Self::Message> {
        ...
        let resulting_element = if status_bar::need_show_status_bar(&wallet_info.extra_info) {
            let status_bar =
                container(status_bar::view_status_bar(&wallet_info.extra_info)).width(Length::Fill);

            Element::new(column![pane_grid, horizontal_rule(1), status_bar])
        } else {
            pane_grid
        };

        resulting_element.map(|msg| TabsMessage::WalletMessage(self.wallet_id, msg))
    }
}
