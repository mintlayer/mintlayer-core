...
mod test {
    use crate::{
        ...
        chain::{
            ...
            transaction::signature::tests::utils::{
                generate_inputs_infos, generate_unsigned_tx, sig_hash_types,
            },
            Destination,
        },
    };
    ...
    fn produce_signature_pub_key_hash_mismatch(#[case] seed: Seed) {
        ...
        let (inputs_info, _) = generate_inputs_infos(&mut rng, 1);
        let inputs_info_refs = inputs_info.iter().map(|info| info.into()).collect::<Vec<_>>();

        let tx = generate_unsigned_tx(&mut rng, &destination, &inputs_info_refs, 2).unwrap();
    ...
    fn produce_and_verify(#[case] seed: Seed) {
        ...
        for (sighash_type, destination) in sig_hash_types().cartesian_product(outpoints.into_iter())
        {
            ...
            let sighash =
                signature_hash(witness.sighash_type(), &tx, &inputs_info_refs, INPUT_NUM).unwrap();
            witness
