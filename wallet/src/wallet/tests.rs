...
fn get_coin_balance_with_inactive(wallet: &DefaultWallet) -> Amount {
    let coin_balance = wallet
        .get_balance(
...
fn get_coin_balance(wallet: &DefaultWallet) -> Amount {
    get_coin_balance_for_acc(wallet, DEFAULT_ACCOUNT_INDEX)
}

fn get_currency_balances(wallet: &DefaultWallet) -> (Amount, Vec<(TokenId, Amount)>) {
...
#[trace]
#[case(Seed::from_entropy())]
fn create_2htlc_refund(#[case] seed: Seed) {
    use common::chain::htlc::HtlcSecret;
    ...
    let secret = HtlcSecret::new_from_rng(&mut rng);
    let spend_key = wallet1.get_new_address(DEFAULT_ACCOUNT_INDEX).unwrap().1;
    let htlc = HashedTimelockContract {
        secret_hash: secret.hash(),
        spend_key: spend_key.into_object(),
        refund_timelock: OutputTimeLock::ForBlockCount(1),
    ...
    // Htlc is available as utxo to spend
    let mut create_htlc_utxos = wallet1
        .get_utxos(
            DEFAULT_ACCOUNT_INDEX,
            UtxoType::Htlc.into(),
            UtxoState::Confirmed.into(),
        ...
        .unwrap();
    assert_eq!(create_htlc_utxos.len(), 1);
    let (_, output, _) = create_htlc_utxos.pop().unwrap();
    match output {
        TxOutput::Htlc(actual_output_value, actual_htlc) => {
            assert_eq!(actual_output_value, output_value);
            assert_eq!(htlc, *actual_htlc);
