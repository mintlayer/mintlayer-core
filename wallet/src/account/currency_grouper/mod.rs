...
pub fn group_utxos_for_input_by_destination<T: std::fmt::Debug, Grouped: Clone>(
    items: impl Iterator<Item = T>,
    get_tx_output: impl Fn(&T) -> &TxOutput,
    mut combiner: impl FnMut(&mut Grouped, &T, Amount) -> WalletResult<()>,
    init: Grouped,
) -> WalletResult<BTreeMap<Destination, Grouped>> {
    let mut result: BTreeMap<Destination, Grouped> = BTreeMap::new();
    ...
    for item in items {
        ...
        if let Some(destination) = destination {
            match currency {
                Currency::Coin => {
                    let for_destination = result.entry(destination).or_insert_with(|| init.clone());
