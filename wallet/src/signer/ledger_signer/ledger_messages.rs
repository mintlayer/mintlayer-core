...
fn encode_path(path: &[u32]) -> Vec<u8> {
    let mut buff = vec![path.len() as u8];

    for x in path {
        buff.extend(x.to_be_bytes());
    }
...
async fn send_chunked<L: Exchange>(
    ...
    while let Some(chunk) = chunks.next() {
        ...
        eprintln!("sending chunk: {} {:?} {:?}", chunk.len(), chunk, msg_buf);
...
pub async fn sign_challenge<L: Exchange>(
    ...
    let sig_len = *resp.get(0).ok_or(LedgerError::InvalidResponse)? as usize;
...
pub async fn get_app_name<L: Exchange>(ledger: &mut L) -> Result<Vec<u8>, ledger_lib::Error> {
    let mut msg_buf = vec![];
    msg_buf.extend([CLA, Ins::APP_NAME, 0, P2::DONE]);

    ledger.exchange(&msg_buf, Duration::from_millis(100)).await
}

pub async fn check_current_app<L: Exchange>(ledger: &mut L) -> Result<(), LedgerError> {
    let mut msg_buf = vec![];
    msg_buf.extend([CLA, Ins::APP_NAME, 0, P2::DONE]);

    let resp = exchange_message(ledger, &msg_buf).await.unwrap();
...
pub async fn get_extended_public_key<D: Exchange>(
    ...
    let pk_len = *resp.get(0).ok_or(LedgerError::InvalidResponse)? as usize;
    ...
    return Ok(ExtendedPublicKey::new(extended_public_key));
}
...
pub async fn sign_tx<L: Exchange>(
    ...
    loop {
        ...
        msg_buf.clear();
        msg_buf.extend([CLA, Ins::SIGN_TX, P1::TX_SIG, P2::DONE, 0]);
        resp = exchange_message(ledger, &msg_buf).await?;
    }
