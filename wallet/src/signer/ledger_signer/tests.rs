...
use ledger_lib::{
    ...
    Device, Transport, DEFAULT_TIMEOUT,
};
use logging::log;
...
use crate::signer::{
    ledger_signer::{
        ...
        speculus::{
            Action, Button, Display, Driver, DriverMode, GenericDriver, GenericHandle, Handle,
            Model, Options,
        },
        LProvider, LedgerError, LedgerSigner,
    },
    tests::{
        generic_tests::{test_sign_transaction_generic, test_sign_transaction_intent_generic},
        no_another_signer,
    },
    ...
    loop {
        tokio::select! {
            _ = sleep(Duration::from_millis(100)) => {
                eprintln!("pressing button! {}", start.elapsed().as_millis());
            ...
            msg = finish_rx.recv() => {
                match msg {
                    Some(ControlMessage::Finish) => {
                        ...
                        // perform exit action
                        driver.exit(handle).await.unwrap();
                        break;
                    }
                    None => {
...
async fn setup(
    ...
    let driver = if docker {
        GenericDriver::new(DriverMode::Docker).unwrap()
    } else {
        GenericDriver::new(DriverMode::Podman).unwrap()
    };
    ...
    let mut handle = driver.run(&app_path, opts.clone()).await.unwrap();

    driver.wait_start(&mut handle).await.unwrap();
    println!("Emulator is running...");

    // FIXME: instead of sleeping try to connect and wait for a successfull connection to the
    // docker
...
async fn test_account_extended_public_key() {
    let (auto_clicker, finish_tx, make_ledger_signer) = setup(1, false, false).await;
...
async fn test_sign_message(#[case] seed: Seed) {
    ...
    let (auto_clicker, finish_tx, make_ledger_signer) = setup(2, false, false).await;

    test_sign_message_generic(&mut rng, make_ledger_signer, no_another_signer()).await;
...
async fn test_sign_transaction_intent(#[case] seed: Seed) {
    ...
    let (auto_clicker, finish_tx, make_ledger_signer) = setup(3, false, false).await;
...
async fn test_sign_transaction(
    ...
    let (auto_clicker, finish_tx, make_ledger_signer) = setup(4, false, false).await;
...
async fn test_fixed_signatures2(
    ...
    let (auto_clicker, finish_tx, make_ledger_signer) = setup(6, true, false).await;
...
async fn test_sign_message_sig_consistency(#[case] seed: Seed) {
    ...
    let (auto_clicker, finish_tx, make_ledger_signer) = setup(7, true, false).await;
...
async fn test_sign_transaction_intent_sig_consistency(#[case] seed: Seed) {
    ...
    let (auto_clicker, finish_tx, make_ledger_signer) = setup(8, true, false).await;
...
async fn test_sign_transaction_sig_consistency(
    ...
    let (auto_clicker, finish_tx, make_ledger_signer) = setup(9, true, false).await;
