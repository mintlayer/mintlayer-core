    ...
    P: LedgerFinder<Ledger = L>,
{
        ...
        loop {
            match get_app_name(&mut *client).await {
                ...
                Err(err) => {
                    return (
                        db_tx,
                        Err(SignerError::LedgerError(LedgerError::DeviceError(
                            err.to_string(),
                        ))),
                    )
                }
    ...
    {
        let (db_tx, res) = self.check_session(db_tx, key_chain).await;
        if let Err(e) = res {
            return (db_tx, Err(e));
        }
    ...
    P: Send + Sync + LedgerFinder<Ledger = L>,
{
    async fn sign_tx<T: WalletStorageReadUnlocked + Send>(
        ...
        let (inputs, standalone_inputs) = match to_ledger_input_msgs(&ptx, key_chain, &db_tx) {
            Ok(x) => x,
            Err(e) => return (db_tx, Err(e)),
        };
        let outputs = self.to_ledger_output_msgs(&ptx);
        let input_commitments = match to_ledger_commitments_msgs(&ptx) {
            Ok(x) => x,
            Err(e) => return (db_tx, Err(e)),
        };
        ...
        let (db_tx, res) = self
            ...
            .await;
        let new_signatures = match res {
            Ok(x) => x,
            Err(e) => return (db_tx, Err(e)),
        };

        let input_commitments =
            match ptx.make_sighash_input_commitments_at_height(&self.chain_config, block_height) {
                Ok(x) => x,
                Err(e) => return (db_tx, Err(e.into())),
            };

        let res = itertools::process_results(
            izip!(
                ptx.witnesses(),
                ptx.input_utxos(),
                ptx.destinations(),
        ...
        let (witnesses, prev_statuses, new_statuses) = match res {
            Ok(x) => x,
            Err(e) => return (db_tx, Err(e)),
        };

        let ptx = match ptx.with_witnesses(witnesses) {
            Ok(x) => x,
            Err(e) => return (db_tx, Err(e.into())),
        };

        (db_tx, Ok((ptx, prev_statuses, new_statuses)))
    }
    ...
    async fn sign_challenge<T: WalletStorageReadUnlocked + Send>(
        ...
        let data = match key_chain.find_public_key(destination) {
            Some(FoundPubKey::Hierarchy(xpub)) => {
                ...
                let res = self
                    .perform_ledger_operation(
                        async move |client| {
                            sign_challenge(
                                client, chain_type, &address_n, // addr_type,
                                &message,
                            )
                            .await
                        },
            ...
            None => return (db_tx, Err(SignerError::DestinationNotFromThisWallet)),
        };
...
async fn check_public_keys_against_key_chain<L: Exchange, T: WalletStorageReadLocked>(
    ...
    let expected_pk =
        match fetch_extended_pub_key(client, chain_config, key_chain.account_index()).await {
            Ok(x) => x,
            Err(e) => return (db_tx, Err(e.into())),
        };
