...
async fn find_ledger_device_from_db(
...
) -> SignerResult<(LedgerHandle, LedgerData)> {
    ...
    if let Some(HardwareWalletData::Ledger(_data)) = db_tx.get_hardware_wallet_data()? {
        // let selected = SelectedDevice {
        //     device_id: data.device_id,
        // };

        find_ledger_device().await.map_err(SignerError::LedgerError)
    } else {
        Err(SignerError::LedgerError(
            LedgerError::MissingHardwareWalletData,
        ))
    }
}

async fn find_ledger_device() -> Result<(LedgerHandle, LedgerData), LedgerError> {
...
async fn check_public_keys_against_key_chain<L: Exchange>(
    ...
    if let Some(_data) = db_tx.get_hardware_wallet_data()? {
        //
    }
