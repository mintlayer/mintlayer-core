...
use crate::{
    ...
    commands::{CLIWalletCommand, ConsoleCommand, WalletCommand},
...
pub fn get_repl_command() -> Command {
    ...
    let mut repl_command = CLIWalletCommand::augment_subcommands(repl_command);
...
fn cli_parse_input(
    line: &str,
    repl_command: &Command,
) -> Result<Option<CLIWalletCommand>, WalletCliError> {
    let line = line.trim();
    if line.is_empty() || line.starts_with('#') {
        return Ok(None);
    }
    ...
    let mut matches = repl_command
        ...
        .try_get_matches_from(args)
        .map_err(WalletCliError::InvalidCommandInput)?;
    let command = CLIWalletCommand::from_arg_matches_mut(&mut matches)
        .map_err(WalletCliError::InvalidCommandInput)?;
    Ok(Some(command))
}
