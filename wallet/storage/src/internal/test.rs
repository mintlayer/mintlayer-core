...
fn storage_get_default_version_in_tx() {
    utils::concurrency::model(|| {
        tokio_test::block_on(async {
            ...
            let vtx = store.transaction_ro().await.unwrap().get_storage_version().unwrap();
...
fn compare_encrypt_and_decrypt_root_key(#[case] seed: Seed) {
    utils::concurrency::model(move || {
        tokio_test::block_on(async {
            ...
            {
                let mut db_tx = store.transaction_rw_unlocked(None).await.unwrap();
                db_tx.set_root_key(&key_content).unwrap();
                db_tx.commit().unwrap();
            }

            {
                let db_tx = store.transaction_ro_unlocked().await.unwrap();
            ...
            store.encrypt_private_keys(&Some(new_password.clone())).await.unwrap();
            ...
            {
                let error = store.transaction_ro_unlocked().await;
                assert_eq!(error.err(), Some(crate::Error::WalletLocked));
            }
            {
                let error = store.transaction_rw_unlocked(None).await;
                assert_eq!(error.err(), Some(crate::Error::WalletLocked));
            }
