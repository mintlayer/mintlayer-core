    ...
    B: storage::Backend + 'static,
{
    ...
    fn get_address_balances(&self) -> Result<BTreeMap<Destination, Amount>, ControllerError<T>> {
        self.wallet
            .get_address_balances(self.account_index)
            .map_err(ControllerError::WalletError)
    }
    ...
    /// The used boolean field is true if the address is used, false is otherwise.
    ...
    pub fn get_addresses_with_usage(
        ...
        fn map_addresses<'a, I>(
            iter: I,
            balances: &BTreeMap<Destination, Amount>,
        ) -> Vec<AddressInfo>
        where
            I: Iterator<
                Item = (
                    ChildNumber,
                    Address<Destination>,
                    &'a KeychainUsageState,
                    KeyPurpose,
                ),
            >,
        {
            iter.map(|(child_number, address, usage, purpose)| {
        ...
        let get_addresses = |key_purpose| -> Result<_, ControllerError<T>> {
            ...
            Ok(addresses
                .into_iter()
                .map(move |(child_number, address)| (child_number, address, usage, key_purpose)))
        };

        let balances = self.get_address_balances()?;

        let result = if include_change_addresses {
            map_addresses(
                get_addresses(KeyPurpose::ReceiveFunds)?.chain(get_addresses(KeyPurpose::Change)?),
                &balances,
            )
        } else {
            map_addresses(get_addresses(KeyPurpose::ReceiveFunds)?, &balances)
        };
