...
impl<T: NodeInterface + Clone + Send + Sync + 'static> Controller<T> {
    ...
    /// Encrypts the wallet using the specified password, or removes the existing encryption if the password is `None`.
    ///
    /// # Arguments
    ///
    ...
    /// # Arguments
    ///
    /// * `password` - A `String` representing the old password that was used to encrypt the wallet.
    ///
    /// # Returns
    ///
    /// This method returns an error the password is incorrect
    ...
    /// # Returns
    ///
    /// This method returns an error the wallet is not encrypted.
