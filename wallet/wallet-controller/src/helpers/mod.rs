...
pub async fn fetch_utxo<T: NodeInterface, B: storage::AsyncBackend>(
    rpc_client: &T,
...
async fn fetch_utxo_and_destination<T: NodeInterface, B: storage::AsyncBackend>(
    rpc_client: &T,
    wallet: &RuntimeWallet<B>,
    input: &UtxoOutPoint,
...
pub async fn tx_to_partially_signed_tx<T: NodeInterface, B: storage::AsyncBackend>(
    rpc_client: &T,
    wallet: &RuntimeWallet<B>,
    tx: Transaction,
...
pub async fn fetch_input_infos<T: NodeInterface, B: storage::AsyncBackend>(
    rpc_client: &T,
    wallet: &RuntimeWallet<B>,
...
async fn into_utxo_and_destination<T: NodeInterface, B: storage::AsyncBackend>(
    rpc_client: &T,
    wallet: &RuntimeWallet<B>,
    tx_inp: &TxInput,
    htlc_spending_condition: HtlcSpendingCondition,
