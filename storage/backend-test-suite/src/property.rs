...
fn async_overwrite_and_abort<B: AsyncBackend, F: BackendFn<B>>(backend_fn: Arc<F>) {
    using_proptest(
        file!(),
        backend_fn,
        (gen::key(100), gen::any::<Data>(), gen::any::<Data>())
...
fn async_add_and_delete<B: AsyncBackend, F: BackendFn<B>>(backend_fn: Arc<F>) {
    const NUM_DBS: usize = 5;
    using_proptest(
        file!(),
        backend_fn,
...
fn async_last_write_wins<B: AsyncBackend, F: BackendFn<B>>(backend_fn: Arc<F>) {
    using_proptest(
        file!(),
        backend_fn,
        (
...
fn async_add_and_delete_some<B: AsyncBackend, F: BackendFn<B>>(backend_fn: Arc<F>) {
    const NUM_DBS: usize = 5;
    using_proptest(
        file!(),
        backend_fn,
        ...
        |backend, (entries1, entries2, extra_keys)| {
            tokio_test::block_on(async move {
                ...
                let dbtx = store.transaction_ro().await.unwrap();
                for ent @ (db, key) in
                    entries1.keys().chain(entries2.keys()).chain(extra_keys.iter())
                {
                    let expected =
                        entries2.get(ent).or_else(|| entries1.get(ent)).map(AsRef::as_ref);
                    assert_eq!(
                        dbtx.get(*db, key).unwrap().as_ref().map(|v| v.as_ref()),
                        expected
                    );
...
fn async_add_modify_abort_modify_commit<B: AsyncBackend, F: BackendFn<B>>(backend_fn: Arc<F>) {
    using_proptest(
        file!(),
        backend_fn,
        (
        ...
        |backend, (to_prepopulate, to_abort, to_commit)| {
            tokio_test::block_on(async move {
                ...
                dbtx.apply_actions(MAPID.0, to_prepopulate.into_iter());
                dbtx.commit().unwrap();
                assert_eq!(model, Model::from_async_db(&store, MAPID.0).await);
...
fn async_add_modify_abort_replay_commit<B: AsyncBackend, F: BackendFn<B>>(backend_fn: Arc<F>) {
    using_proptest(
        file!(),
        backend_fn,
        (gen::actions(100, 0..20), gen::actions(100, 0..20)),
        |backend, (initial, actions)| {
            tokio_test::block_on(async move {
                ...
                let initial_model = Model::from_async_db(&store, MAPID.0).await;
                ...
                let modified_model = Model::from_tx(&dbtx, MAPID.0);
                drop(dbtx);
                assert_eq!(Model::from_async_db(&store, MAPID.0).await, initial_model);

                // Apply the same changes again, and check that we get to the same state after commit
                let mut dbtx = store.transaction_rw(None).await.unwrap();
                dbtx.apply_actions(MAPID.0, actions.into_iter());
                dbtx.commit().unwrap();
                assert_eq!(modified_model, Model::from_async_db(&store, MAPID.0).await);
            })
        },
    )
}
...
fn async_db_writes_do_not_interfere<B: AsyncBackend, F: BackendFn<B>>(backend_fn: Arc<F>) {
    using_proptest(
        file!(),
        backend_fn,
        (gen::actions(100, 0..20), gen::actions(100, 0..20)),
        |backend, (actions0, actions1)| {
            tokio_test::block_on(async move {
                ...
                dbtx.apply_actions(MAPID.0, actions0.into_iter());
                dbtx.commit().unwrap();
                let model = Model::from_async_db(&store, MAPID.0).await;
...
fn async_empty_after_abort<B: AsyncBackend, F: BackendFn<B>>(backend_fn: Arc<F>) {
    using_proptest(
        file!(),
        backend_fn,
        (
...
fn async_prefix_iteration<B: AsyncBackend, F: BackendFn<B>>(backend_fn: Arc<F>) {
    using_proptest(
        file!(),
        backend_fn,
        (gen::actions(100, 0..20), gen::actions(100, 0..20)),
...
fn async_post_commit_consistency<B: AsyncBackend, F: BackendFn<B>>(backend_fn: Arc<F>) {
    using_proptest(
        file!(),
        backend_fn,
        gen::actions(100, 0..50),
        |backend, actions| {
            tokio_test::block_on(async move {
                ...
                // The state from the transaction just before committing should de the same as the
                // state of the database after the commit.
                assert_eq!(Model::from_async_db(&store, MAPID.0).await, model);
            })
        },
    )
}
...
async_tests![
    async_add_and_delete,
    async_add_and_delete_some,
    async_add_modify_abort_modify_commit,
    async_add_modify_abort_replay_commit,
    async_db_writes_do_not_interfere,
    async_empty_after_abort,
    async_last_write_wins,
    async_overwrite_and_abort,
    async_post_commit_consistency,
    async_prefix_iteration,
];
