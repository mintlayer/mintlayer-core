...
fn test_scale_version_compatibility_numbers() {
    ...
    let dec = SimpleWrapper::decode(&mut hex!("01").as_slice()).ok();
    assert!(&dec.is_some());
    ...
    // Backward compatibility
    let dec = SimpleWrapper::decode(&mut hex!("02").as_slice()).ok();
    assert!(&dec.is_some());
    assert_eq!(dec, Some(SimpleWrapper(test)));
    ...
    // Backward compatibility
    let dec = SimpleWrapper::decode(&mut hex!("d204").as_slice()).ok();
    assert!(&dec.is_some());
    assert_eq!(dec, Some(SimpleWrapper(test)));
    ...
    // Backward compatibility
    let dec = SimpleWrapper::decode(&mut hex!("2e16").as_slice()).ok();
    assert!(&dec.is_some());
    assert_eq!(dec, Some(SimpleWrapper(test)));
    ...
    // Backward compatibility
    let dec = SimpleWrapper::decode(&mut hex!("78d7c83d").as_slice()).ok();
    assert!(&dec.is_some());
    assert_eq!(dec, Some(SimpleWrapper(test)));
    ...
    // Backward compatibility
    let dec = SimpleWrapper::decode(&mut hex!("9c9bf78f").as_slice()).ok();
    assert!(&dec.is_some());
    assert_eq!(dec, Some(SimpleWrapper(test)));
    ...
    // Backward compatibility
    let dec = SimpleWrapper::decode(&mut hex!("a99874d96fd4c8df").as_slice()).ok();
    assert!(&dec.is_some());
    assert_eq!(dec, Some(SimpleWrapper(test)));
    ...
    // Backward compatibility
    let dec = SimpleWrapper::decode(&mut hex!("45598e2b5942260f").as_slice()).ok();
    assert!(&dec.is_some());
    assert_eq!(dec, Some(SimpleWrapper(test)));
    ...
    // Backward compatibility
    let dec = SimpleWrapper::decode(&mut hex!("6f7d32890204cfda1af9994ef5020080").as_slice()).ok();
    assert!(&dec.is_some());
    assert_eq!(dec, Some(SimpleWrapper(test)));
    ...
    // Backward compatibility
    let dec = SimpleWrapper::decode(&mut hex!("5029f787d210ec973296fe1e23e6729e").as_slice()).ok();
    assert!(&dec.is_some());
    assert_eq!(dec, Some(SimpleWrapper(test)));
}
...
fn test_scale_version_compatibility_array() {
    let test = [
        12345, 67890, 10111213, 141516171, 192021229, 232425262, 293031323, 343536373, 394041424,
        454647484,
    ];
    let enc = SimpleWrapper::encode(&SimpleWrapper(test));
    let bytes_representation =
        hex!("3930000032090100ed489a008b5d6f08ed02720b2e87da0d9b4d7711f5f2791450987c17bc5e191b");
    assert_eq!(&enc, &bytes_representation);
    // Backward compatibility
    let dec = SimpleWrapper::decode(&mut &bytes_representation[..]).ok();
    assert!(&dec.is_some());
