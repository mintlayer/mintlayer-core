...
pub fn encode_witness(
    ...
    let input_infos =
        TxInputsAdditionalInfo::from_tx_addiitonal_info(&chain_config, &additional_info)?;
...
///
/// `key_index` parameter is an index of a public key in the challenge, against which is the signature produced from private key is to be verified.
