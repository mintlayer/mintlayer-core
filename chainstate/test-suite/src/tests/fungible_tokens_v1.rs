...
fn mint_tokens_in_block(
    ...
    let tx_builder = TransactionBuilder::new()
        .add_input(
            TxInput::from_account(nonce, AccountOp::MintTokens(token_id, amount_to_mint)),
            InputWitness::NoSignature(None),
        )
        .add_input(
...
fn unmint_tokens_in_block(
    ...
    let tx_builder = TransactionBuilder::new()
        .add_input(
            TxInput::from_account(nonce, AccountOp::UnmintTokens(token_id)),
            InputWitness::NoSignature(None),
        )
        .add_input(
...
fn check_lockable_supply(#[case] seed: Seed) {
    utils::concurrency::model(move || {
        ...
        let lock_tx = TransactionBuilder::new()
            .add_input(
                TxInput::from_account(nonce, AccountOp::LockTokenSupply(token_id)),
                InputWitness::NoSignature(None),
            )
            .add_input(
                TxInput::from_utxo(mint_tx_id.into(), 0),
...
fn supply_change_fees(#[case] seed: Seed) {
    utils::concurrency::model(move || {
        ...
        let lock_supply_tx = TransactionBuilder::new()
            .add_input(
            ...
            .add_output(TxOutput::Burn(OutputValue::Coin(low_fee)))
            .build();
        let lock_supply_tx_id = lock_supply_tx.transaction().get_id();
        let result = tf.make_block_builder().add_transaction(lock_supply_tx).build_and_process();
...
// Issue a token.
// Then in a single tx try mint some tokens and issue another tx with not enough fee.
// Then try again but with fee that satisfy both mint and issuance.
#[rstest]
#[trace]
#[case(Seed::from_entropy())]
fn token_issue_and_mint_not_enough_fee(#[case] seed: Seed) {
    utils::concurrency::model(move || {
        let mut rng = make_seedable_rng(seed);
        let mut tf = make_test_framework_with_v1(&mut rng);
        let token_min_issuance_fee = tf.chainstate.get_chain_config().token_min_issuance_fee();
