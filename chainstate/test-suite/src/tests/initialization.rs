...
fn genesis_check_ok(num_blocks: u64) {
    utils::concurrency::model(move || {
        ...
        let _tf = TestFramework::builder().with_storage(storage).build();
    })
}
...
fn genesis_check_err(num_blocks: u64) {
    utils::concurrency::model(move || {
        ...
        let storage = {
            let mut tf = TestFramework::builder().with_chain_config(conf0).build();
        ...
        let result = TestFramework::builder()
...
#[test]
fn genesis_check_ok_empty_chain() {
    genesis_check_ok(0);
}
...
fn genesis_check_ok_nonempty_chain(#[case] seed: Seed) {
    genesis_check_ok(make_seedable_rng(seed).gen_range(1..100));
}

#[test]
fn genesis_check_err_empty_chain() {
    genesis_check_err(0);
}

#[test]
fn genesis_check_err_height_1() {
    genesis_check_err(1);
}
