...
use super::{block_invalidator::BlockInvalidator, *};
use crate::{
    detail::{
    ...
    BlockError, OrphanCheckError, TransactionVerificationStrategy,
};
...
pub struct BlockProcessor<'a, S, V> {
    chainstate: &'a mut Chainstate<S, V>,
}

impl<'a, S: BlockchainStorage, V: TransactionVerificationStrategy> BlockProcessor<'a, S, V> {
    pub fn new(chainstate: &'a mut Chainstate<S, V>) -> BlockProcessor<'a, S, V> {
        BlockProcessor { chainstate }
    }
    ...
    fn attempt_to_process_block(
        ...
        let block_index = {
            let chainstate_ref =
                self.chainstate.make_db_tx_ro().map_err(BlockError::from).log_err()?;
        ...
        let integrate_block_result = self.chainstate.with_rw_tx(
        ...
        if let Some(first_invalid_block_id) = first_invalid_block_id {
            ...
            let _result = BlockInvalidator::new(self.chainstate)
                .invalidate_stale_block(&first_invalid_block_id, false)
                .log_err();
        }
    ...
    fn process_orphans_of(
        ...
        while let Some(block_id) = orphan_process_queue.pop_front() {
            let orphans =
                self.chainstate.orphan_blocks_pool_mut().take_all_children_of(&block_id.into());
            ...
            block_errors.into_iter().for_each(|e| {
                match &self.chainstate.custom_orphan_error_hook() {
                    Some(handler) => handler(&e),
                    None => {
                        logging::log::error!("Failed to process a chain of orphan blocks: {}", e)
                    }
                }
            });
        }
    ...
    pub fn remove_orphans_of(&mut self, block_id: &Id<Block>) {
        let mut orphan_process_queue: VecDeque<_> = vec![*block_id].into();
        while let Some(block_id) = orphan_process_queue.pop_front() {
            let orphans =
                self.chainstate.orphan_blocks_pool_mut().take_all_children_of(&block_id.into());
    ...
    pub fn process_block_and_related_orphans(
        &mut self,
        block: WithId<Block>,
        block_source: BlockSource,
    ) -> Result<Option<BlockIndex>, BlockError> {
        ...
        if let Some(ref bi) = result {
            ...
            self.chainstate.set_is_initial_block_download_finished(
                self.chainstate.is_fresh_block(&bi.block_timestamp()),
            );
        }
    ...
    fn broadcast_new_tip_event(&self, new_block_index: &Option<BlockIndex>) {
        match new_block_index {
            Some(ref new_block_index) => {
                ...
                self.chainstate
                    .events_controller()
                    .broadcast(ChainstateEvent::NewTip(new_id, new_height))
            }
            None => (),
        }
    }
    ...
    fn check_legitimate_orphan(
        ...
        let chainstate_ref = self.chainstate.make_db_tx_ro().map_err(OrphanCheckError::from)?;
    ...
    fn new_orphan_block(&mut self, block: WithId<Block>) -> Result<(), OrphanCheckError> {
        match self.chainstate.orphan_blocks_pool_mut().add_block(block) {
            Ok(_) => Ok(()),
            Err(err) => (*err).into(),
        }
    }
