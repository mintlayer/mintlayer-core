...
mod test {
    ...
    fn derivation_private_key() {
        ...
        for (path, path_encoded, secret, public, chaincode) in test_vec {
    ...
    fn derivation_public_key() {
        ...
        for (path, path_encoded, public, chaincode, is_success) in test_vec {
            let path = DerivationPath::from_str(path).unwrap();
            let pk = master_pub_key.clone().derive_path(&path);
            if !is_success {
