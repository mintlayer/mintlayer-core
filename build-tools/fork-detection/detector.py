...
NETWORK_TYPE_CHOICES = ["mainnet", "testnet"]
...
class Handler():
    ...
    def do_full_sync(self):
        ...
        # Note: we need at least chainstate=debug to be able to catch the "Stale block received" line.
        # Ideally thought we'd want to collect the entire debug log, just in case.
        # But the tx_verifier's debug log is too noisy, making the log file gain 100Mb in less
        # than a minute. With "debug,tx_verifier=info" it's under 20Mb per minute, but this still
        # results in a 3Gb log file after full sync on mainnet. So we stick with "info".
        node_proc_env["RUST_LOG"] = "info,chainstate=debug"
