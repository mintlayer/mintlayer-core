...
class Handler():
    def __init__(self, args):
        # Note: the first item will correspond to genesis, with the timestamp being real and
        # the target bogus.
        print(f"Reading input data from {args.input_file}")
        all_timestamps, all_targets = zip(*load_timestamps_targets(args.input_file))
        ...
        self.all_targets = all_targets[1:]
        self.all_time_diffs = [t2 - t1 for t1, t2 in zip(all_timestamps, all_timestamps[1:])]
...
def load_timestamps_targets(input_file):
    with open(input_file, 'r') as file:
        csv_reader = csv.reader(file)

        load_timestamps_targets = []
        for timestamp, target in csv_reader:
            load_timestamps_targets.append((int(timestamp), int(target)))
        return load_timestamps_targets
