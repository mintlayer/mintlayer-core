...
fn read_config_override_values() {
    ...
    let min_tx_relay_fee_per_byte = 321;
    ...
    let options = RunOptions {
        ...
        min_tx_relay_fee_per_byte: Some(min_tx_relay_fee_per_byte),
    };
    let config = NodeConfigFile::read(&chain_config, &config_path, &options).unwrap();
    ...
    assert_eq!(
        config.mempool.unwrap().min_tx_relay_fee_per_byte,
        Some(min_tx_relay_fee_per_byte)
    );
